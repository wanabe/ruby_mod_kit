...
module RubyModKit
  ...
  class Generation
    ...
    def perform_missions
      @missions.each do |mission|
        case mission
        when Mission::IvarArg
          def_node = @root_node[mission.offset, Prism::DefNode]
          ...
          self[src_offset, 0] = "#{" " * indent}#{mission.modify_script}\n"
        when Mission::TypeParameter
          def_node = @root_node[mission.offset, Prism::DefNode]
          raise RubyModKit::Error, "DefNode not found" if !def_node || !def_node.prism_node.is_a?(Prism::DefNode)

          parameter_node = @root_node[mission.offset]
          ...
          self[src_offset, 0] = "#{" " * indent}# @rbs #{parameter_node.name}: #{mission.modify_script}\n"
        else
          raise RubyModKit::Error, "Unexpected type #{mission.class}"
        end
      end
      @missions.clear
    end
