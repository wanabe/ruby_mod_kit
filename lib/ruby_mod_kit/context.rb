...
module RubyModKit
  ...
  class Context
    ...
    # @rbs return: Context
    def generate_next
      Context.new(@script, mod_data: @mod_data)
    end

    # @rbs return: void
    def self.correct_and_collect_repeatedly
      until @context.parse_result.errors.empty?
        previous_error_count = @context.parse_result.errors.size
        @context.correct_and_collect
        @context = @context.generate_next
        next unless previous_error_count > 0 && previous_error_count <= @context.parse_result.errors.size

        @context.parse_result.errors.each do |parse_error|
          warn(
            ":#{parse_error.location.start_line}:#{parse_error.message} (#{parse_error.type})",
            @context.parse_result.source.lines[parse_error.location.start_line - 1],
