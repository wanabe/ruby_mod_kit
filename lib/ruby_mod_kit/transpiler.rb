...
module RubyModKit
  ...
  class Transpiler
    ...
    def insert_diff(src_offset, new_diff)
      array_index = @diffs.find_index do |(offset, _)|
        src_offset < offset
      end
      @diffs.insert(array_index || -1, [src_offset, new_diff])
    end
    ...
    def insert_mod_data(src_offset, type, modify_script)
      array_index = @mod_data.find_index do |(offset, _)|
        src_offset < offset
      end
      @mod_data.insert(array_index || -1, [src_offset, type, modify_script])
    end
  end
end
