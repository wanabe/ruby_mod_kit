      ...
      loop do
        ...
        parse_errors.each do |parse_error|
          ...
          when :unexpected_token_ignore
            ...
            part = @dst[dst_index(last_parameter_offset)...dst_index(right_offset)]&.sub(/\s*=>\s*\z/, "")
            ...
            if overload_methods
              ...
              overload_methods[overload_id][def_node] << part
            end
        ...
        overload_methods&.each do |(_, name), def_node_part_pairs|
          ...
          src_index = parse_result.source.offsets[first_def_node.prism_node.location.start_line - 1]
          script = +""
          ...
          script << "  end\nend\n\n"
          indent = first_def_node.prism_node.location.start_offset - src_index
          script.gsub!(/^(?=.)/, " " * indent)
          self[src_index, 0] = script
        end
